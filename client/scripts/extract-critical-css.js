...
async function extractCriticalCSS() {
  ...
  try {
    ...
    if (cssFiles.length === 0) {
      console.log('❌ No CSS files found in dist. Please build the project first.');
      return;
    }

    const cssFile = path.join(distPath, cssFiles[0]);
    ...
    jsFiles.forEach(jsFile => {
      jsContent += fs.readFileSync(path.join(distPath, jsFile), 'utf8');
    });
